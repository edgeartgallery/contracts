...
// File: contracts/artwork/EdgeNFT.sol
...
contract EdgeNFT is ERC721Pausable, AccessControl, Ownable {
    ...
    function mint(address to, string memory _tokenURI, uint256 amount) public returns (uint256 tokenId) {
        require(feeToken.balanceOf(msg.sender) >= amount, "You don't have enough token for creation.");
        require(amount >= mintFeeAmount, "You don't have enough token for creation.");
        require(feeToken.transferFrom(msg.sender, mintFeeAddr, amount), "Creation failed due to failed transfer.");
        tokenId = nextTokenId;
        _mint(to, tokenId);
        nextTokenId++;
